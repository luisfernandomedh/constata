 ...
 */
function codigoSintetico(largo) {
 ...
 */
export function desactivarEnlace(url) {
...
export function tieneEnlaces(texto) {
...
export function anonimizar(texto) {
    ...
    if (huboEnlaces)
        reemplazos.push("enlaces desactivados y sin su ruta");
    ...
    const conCodigos = resultado.replace(/((?:c[oó]digo|clave|pin|otp)\w*[^.!?\n]{0,80}?)(\d{4,8})\b/giu, (_todo, antes, digitos) => `${antes}${CENTINELA}${codigoSintetico(digitos.length)}${CENTINELA}`);
    ...
    for (const regla of REGLAS) {
        ...
        const partes = resultado.split(RE_DIVIDIR_URL).flatMap((tramo, i) => i % 2 === 1 ? [tramo] : tramo.split(protegido));
        let cambio = false;
        const nuevas = partes.map((parte) => {
            if (parte.startsWith(CENTINELA) || RE_ES_URL.test(parte))
                return parte;
 ...
 */
export function reactivarEnlaces(texto) {
