...
const acortador = {
    ...
    detectar(ctx) {
        ...
        return {
            id: "enlace-acortado",
            peso: 25,
            explicacion: "El mensaje usa un acortador de enlaces, que oculta a dónde te lleva realmente. Las empresas serias no acortan sus propios enlaces.",
            evidencia: hallado,
        };
    },
};
const desajusteMarca = {
    ...
    detectar(ctx) {
        ...
        for (const marca of MARCAS) {
            ...
            if (!mencionada)
                continue;
            const coincide = ctx.dominios.some((d) => marca.dominios.some((leg) => d === leg || d.endsWith(`.${leg}`)));
            if (coincide)
                continue;
            return {
                id: "desajuste-marca-enlace",
    ...
    detectar(ctx) {
        for (const d of ctx.dominios) {
            if (d.startsWith("xn--")) {
                return {
                    id: "caracteres-enganosos",
                    peso: 40,
                    explicacion: "El dominio del enlace usa caracteres de otro alfabeto disfrazados de letras latinas. Es una técnica para imitar el nombre de un sitio real.",
                    evidencia: d,
                };
            }
        }
        if (/[а-яА-ЯαβγδεΑΒΓ]/.test(ctx.texto) && ctx.dominios.length > 0) {
            return {
                id: "caracteres-enganosos",
                peso: 30,
                explicacion: "El mensaje mezcla letras de otro alfabeto con texto en español, algo típico de imitaciones de marcas.",
            };
        }
        return null;
    },
...
const urgencia = {
    ...
    detectar(ctx) {
        ...
        return {
            ...
            explicacion: "El mensaje te apura con un plazo o una amenaza de perder la cuenta. La prisa impide pensar, y por eso es la herramienta favorita del estafador.",
        };
    },
};
...
const pideCredenciales = {
    ...
    detectar(ctx) {
        if (!RE_CREDENCIALES.some((re) => re.test(ctx.normalizado)))
            return null;
        return {
            id: "pide-credenciales",
            peso: 50,
            explicacion: "El mensaje pide un código, una clave o datos de tarjeta. Ningún banco ni empresa legítima pide eso por mensaje. Nunca. Esta sola señal basta para no responder.",
        };
    },
};
const RE_PREMIO = [
...
const premioInesperado = {
    ...
    detectar(ctx) {
        ...
        return {
            ...
            explicacion: "El mensaje ofrece un premio, un pago o un paquete que no esperabas. Si no participaste en nada, no hay nada que reclamar.",
        };
    },
};
const dominioReciente = {
    id: "dominio-imitador",
    detectar(ctx) {
        for (const d of ctx.dominios) {
            ...
            for (const marca of MARCAS) {
                for (const alias of marca.alias) {
                    ...
                    const esLegitimo = marca.dominios.some((leg) => d === leg || d.endsWith(`.${leg}`));
